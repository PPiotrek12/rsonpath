use crate::rewrite_logging::TOOLING_LOG_TARGET;

pub use rsonpath_lib::query_rewrite::benchmark::{
    benchmark_fixed_query as benchmark_fixed_query_impl, best_equivalent_row, run_benchmark as run_benchmark_impl,
    select_best_rewrite as select_best_rewrite_impl, BenchmarkRow, BestRewriteSelection, FixedQueryBenchmarkConfig,
    RewriteBenchmarkConfig, RewriteBenchmarkError, RewriteBenchmarkReport,
};

pub fn run_benchmark(
    config: &RewriteBenchmarkConfig,
) -> Result<RewriteBenchmarkReport, RewriteBenchmarkError> {
    log::debug!(
        target: TOOLING_LOG_TARGET,
        "run_benchmark: document={} no_schema={} schema={:?}",
        config.document_file,
        config.no_schema,
        config.schema_file
    );
    let report = run_benchmark_impl(config)?;
    log::debug!(
        target: TOOLING_LOG_TARGET,
        "run_benchmark: {} candidates, {} benchmark rows",
...
pub fn select_best_rewrite(
    config: &RewriteBenchmarkConfig,
) -> Result<BestRewriteSelection, RewriteBenchmarkError> {
    log::debug!(
        target: TOOLING_LOG_TARGET,
        "select_best_rewrite: document={} no_schema={} schema={:?}",
        config.document_file,
        config.no_schema,
        config.schema_file
    );
    let selection = select_best_rewrite_impl(config)?;
    log::debug!(
        target: TOOLING_LOG_TARGET,
        "select_best_rewrite: best len={} role={} throughput={:.2} GB/s",
        selection.query.len(),
        selection.row.role,
        selection.row.throughput_bytes_per_s / 1_000_000_000.0
    );
    Ok(selection)
}

pub fn benchmark_fixed_query(
    config: &FixedQueryBenchmarkConfig,
) -> Result<BenchmarkRow, RewriteBenchmarkError> {
    log::debug!(
        target: TOOLING_LOG_TARGET,
        "benchmark_fixed_query: role={} document={}",
        config.role,
        config.document_file
    );
    let row = benchmark_fixed_query_impl(config)?;
    log::debug!(
        target: TOOLING_LOG_TARGET,
        "benchmark_fixed_query: role={} matches_baseline={} mean={:.3} ms",
        row.role,
        row.matches_baseline,
        row.exec_mean_ns as f64 / 1_000_000.0
    );
    Ok(row)
}
